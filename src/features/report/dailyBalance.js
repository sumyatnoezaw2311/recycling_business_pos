...
export const fetchDailyBalance = createAsyncThunk(
    'report/fetchDailyBalance',
    async({startDate,endDate,pageNo})=>{
        let urlParam = `?${startDate && `start_date=${startDate}`}
        &${endDate && `end_date=${endDate}`}
        &${pageNo && `page=${pageNo}`}`;
        try{
            const response = await Axios.get(`${BASE_URL}/reports/daily${urlParam}`,
                {
                    headers: HEADERS()
                }
            )
...
const dailyBalanceSlice = createSlice({
    ...
    extraReducers: (builder)=>{
        builder
        .addCase(fetchDailyBalance.pending, (state)=>{
            state.loading = true
            state.data = []
            state.error = null
            return state
        })
        .addCase(fetchDailyBalance.fulfilled, (state,action)=>{
            state.loading = false
            state.data = action.payload
            state.error = null
            return state
        })
        .addCase(fetchDailyBalance.rejected, (state,action)=>{
            state.loading = false
            state.data = []
